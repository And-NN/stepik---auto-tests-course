...
бывает, что это требует слишком много усилий. Поэтому мы будем перезапускать упавший тест, чтобы еще раз убедиться, что
...
проблемные тесты.﻿﻿
Дополнительно мы указали параметр "--tb=line", чтобы сократить лог с результатами теста. Можете почитать подробнее
про настройку вывода в документации PyTest https://docs.pytest.org/en/latest/usage.html#modifying-python-traceback-printing
...
# rerunfailures позволяет перезапускать и отдельные тесты тоже https://github.com/pytest-dev/pytest-rerunfailures